...
namespace RadioactiveBunnies
{
    ...
    {
        public int LastLocationRow { get; set; }
        public int LastLocationCol { get; set; }
    ...
    public class RadioactiveBunnies
    {
        ...
        private static void BunniesMultiplication(char[][] matrix, PlayerInfo playerInfo)
        {
            ...
            foreach (Cell cellsWithBunny in cellsWithBunnies)
            {
                ...
                if (matrix[cellRow][Math.Max(cellCol - 1, 0)].Equals('P')) // left
                {
                ...
                else if (matrix[cellRow][Math.Min(cellCol + 1, matrix[cellRow].Length - 1)].Equals('P')) // right
                {
                    if (playerInfo.IsDead == false)
                    {
                        playerInfo.IsDead = true;
                ...
                else if (matrix[Math.Max(cellRow - 1, 0)][cellCol].Equals('P')) // up
                {
                    if (playerInfo.IsDead == false)
                    {
                        playerInfo.IsDead = true;
                ...
                else if (matrix[Math.Min(cellRow + 1, matrix.Length - 1)][cellCol].Equals('P')) // down
                {
                    if (playerInfo.IsDead == false)
                    {
                        playerInfo.IsDead = true;
